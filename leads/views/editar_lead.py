...
from ..models import Leads, Persona, Observaciones, Telefonos, UsuarioReporte, AcuerdoPago, FechasPago, ReporteGrafico
...
def editar_lead(request, id_lead):
    ...
    imagenes_reportes = ReporteGrafico.objects.filter(id_lead=lead) # NUEVO: Traemos los archivos/imágenes
    ...
    return render(request, 'leads/editar_lead.html', {
        ...
        'telefonos': telefonos,
        'reportes': reportes,
        'imagenes_reportes': imagenes_reportes, # Enviamos al contexto
        'acuerdo': acuerdo,
...
# FUNCIÓN PARA PROCESAR LOS PAGOS INLINE (Se mantiene intacta)
def registrar_pago(request, id_fecha_pago):
    if request.method == 'POST':
        pago = get_object_or_404(FechasPago, id_fecha_pago=id_fecha_pago)
        id_lead = request.POST.get('id_lead')
        valor_ingresado = request.POST.get('valor_pagado')
        fecha_ingresada = request.POST.get('fecha_pagado')

        if valor_ingresado and fecha_ingresada:
            pago.valor_pagado = valor_ingresado
            pago.fecha_pagado = fecha_ingresada
            
            if float(valor_ingresado) >= float(pago.valor_fecha_pago):
                pago.estado_pago = 'PAGADO'
            elif float(valor_ingresado) > 0:
                pago.estado_pago = 'ABONO'
            else:
                pago.estado_pago = 'PENDIENTE'
                
            pago.save()

        return redirect('editar_lead', id_lead=id_lead)
