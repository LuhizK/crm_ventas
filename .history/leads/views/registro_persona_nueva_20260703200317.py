        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # 2. Asignamos un usuario temporal (mientras implementamos Login)
                    # Tomamos el primer usuario registrado en la base de datos
                    asesor_temporal = User.objects.first() 
                    
                    # 3. Crear el Lead asociado de forma atómica
                    Lead.objects.create(
                        persona=nueva_persona,
                        telefono=telefono,
                        observacion=form.cleaned_data['observacion_referencia'],
                        asesor=asesor_temporal, 
                        estado='NUEVO'
                    )
