...
def registro_plataformas(request, id_lead):
    # 1. SEGURIDAD: Buscamos que el Lead realmente exista en la base de datos.
    # Si alguien pone un ID falso en la URL, Django lanza un error 404 automático.
    lead_actual = get_object_or_404(Leads, id_lead=id_lead)

    if request.method == 'POST':
        # CONCEPTO NUEVO: request.FILES
        # Cuando envías archivos (PDFs/Imágenes), no viajan en request.POST, viajan en request.FILES.
        # Debes pasarle AMBOS al formulario, o los archivos serán ignorados.
        form = PlataformasReporteForm(request.POST, request.FILES)

        if form.is_valid():
            try:
                with transaction.atomic():
                    # Definimos los prefijos exactos que usamos en el forms.py
