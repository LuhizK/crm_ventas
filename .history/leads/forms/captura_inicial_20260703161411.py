...
class CapturaTelefonoForm(forms.Form):
    ...
    def clean_numero_marcado(self):
        # Esta función la ejecuta Django automáticamente cuando le das a guardar.
        # Sirve para "limpiar" el dato antes de que el Chef (la vista) lo use.
        numero = self.cleaned_data.get('numero_marcado')
        
        # Le quitamos los espacios en blanco al inicio o al final por si el asesor copió y pegó mal
        numero_limpio = numero.strip()
        
        # Validamos que solo tenga números (y opcionalmente un '+' al inicio)
